const es = {
  // App
  appName: 'Boardgame Helper',
  appTagline: 'Tu asistente de juegos de mesa',

  // Navigation
  home: 'Inicio',
  back: 'Volver',
  settings: 'Configuración',

  // Home Page
  searchPlaceholder: 'Buscar juegos...',
  allGames: 'Todos los juegos',
  noGamesFound: 'No se encontraron juegos',
  players: 'jugadores',
  estimatedTime: 'Tiempo estimado',

  // Player Setup
  playerSetup: 'Configurar Jugadores',
  addPlayer: 'Agregar jugador',
  removePlayer: 'Eliminar',
  playerName: 'Nombre del jugador',
  playerPlaceholder: 'Jugador',
  startGame: 'Iniciar Juego',
  minPlayersWarning: 'Se necesitan al menos {min} jugadores',
  maxPlayersWarning: 'Máximo {max} jugadores',

  // Game Common
  round: 'Ronda',
  score: 'Puntuación',
  totalScore: 'Puntuación Total',
  winner: '¡Ganador!',
  gameOver: '¡Fin del Juego!',
  newGame: 'Nuevo Juego',
  playAgain: 'Jugar de nuevo',
  endGame: 'Terminar Juego',
  confirmEnd: '¿Estás seguro de que quieres terminar el juego?',
  yes: 'Sí',
  no: 'No',
  cancel: 'Cancelar',
  confirm: 'Confirmar',
  nextRound: 'Siguiente Ronda',
  finishRound: 'Finalizar Ronda',
  restartGame: 'Reiniciar Juego',
  confirmRestart: '¿Reiniciar el juego? Se perderá todo el progreso.',
  editScores: 'Editar Puntuación',
  namesOptionalHint: 'Los nombres se registrarán en BGG tal como los ingreses. Déjalos en blanco para registrarlos como "Jugador 1", "Jugador 2", etc.',
  houseRules: 'Reglas de la Casa',
  houseRulesDesc: 'Variaciones de reglas opcionales para este juego.',
  hld_swissShears: 'Tijeras Suizas',

  // Share Card
  shareResults: 'Compartir Resultados',
  download: 'Descargar',
  share: 'Compartir',
  shareTitle: 'Resultados de {game}',
  shareText: '¡Mira los resultados de nuestra partida de {game}!',
  datePlayed: 'Jugado el',
  finalStandings: 'Clasificación Final',

  // Flip 7
  flip7_name: 'Flip 7',
  flip7_description: '¡Voltea cartas y acumula puntos sin pasarte!',
  flip7_hit: 'Voltear',
  flip7_stay: 'Quedarse',
  flip7_unstay: 'Deshacer Quedar',
  flip7_bust: '¡Eliminado!',
  flip7_cardValue: 'Valor de carta',
  flip7_multiplier: 'Multiplicador',
  flip7_modifier: 'Modificador',
  flip7_bonus7: 'Bonus 7 cartas',
  flip7_roundScore: 'Puntuación de ronda',
  flip7_addCard: 'Agregar carta',
  flip7_numberCards: 'Cartas de número',
  flip7_secondChance: 'Segunda oportunidad',
  flip7_targetScore: 'Puntuación objetivo: {target}',

  // Happy Little Dinosaurs
  hld_name: 'Happy Little Dinosaurs',
  hld_description: '¡Sobrevive los desastres y escapa por la Ruta de escape!',
  hld_chooseDinosaur: 'Elige tu dinosaurio',
  hld_escapeRoute: 'Ruta de Escape',
  hld_disasterArea: 'Área de Desastres',
  hld_natural: 'Natural',
  hld_predator: 'Depredador',
  hld_emotional: 'Emocional',
  hld_meteor: 'Meteoro',
  hld_disasterType: 'Tipo de desastre',
  hld_pointCard: 'Carta de puntos',
  hld_traitBonus: 'Bonus de rasgo',
  hld_instantCards: 'Cartas Instantáneas',
  hld_playInstant: 'Jugar carta instantánea',
  hld_searchCards: 'Buscar cartas...',
  hld_roundWinner: 'Ganador de la ronda',
  hld_roundLoser: 'Perdedor de la ronda',
  hld_takesDisaster: 'recibe el desastre',
  hld_eliminated: '¡Eliminado!',
  hld_eliminatedDesc: '{player} ha sido eliminado por acumular 3 desastres',
  hld_wins: '¡{player} gana!',
  hld_reached50: '¡Ha llegado a 50 puntos en la Ruta de Escape!',
  hld_lastStanding: '¡Es el último dinosaurio en pie!',
  hld_hazardTokens: 'Fichas de Peligro',
  hld_scoringPhase: 'Fase de Puntuación',

  // Language
  language: 'Idioma',
  spanish: 'Español',
  english: 'English',

  // Munchkin Adventure Time
  munchkin_description: '¡Derriba puertas, pelea monstruos y agarra tesoro — el primero en llegar al Nivel 10 gana!',
  munchkin_level: 'Nivel',
  munchkin_kill: '+Matar',
  munchkin_sell: '+Vender',
  munchkin_levelCard: '+Carta',
  munchkin_curse: '−Maldición',
  munchkin_setLevel: 'Fijar Nivel',
  munchkin_chooseCharacter: 'Elige tu personaje',
  munchkin_chooseGender: 'Elige género',
  munchkin_male: 'Masculino',
  munchkin_female: 'Femenino',
  munchkin_noGender: 'Sin género',
  munchkin_human: 'Humano',
  munchkin_superMunchkin: 'Super Munchkin',
  munchkin_battle: 'Batalla',
  munchkin_yourSide: 'Tú',
  munchkin_monsterSide: 'Monstruo',
  munchkin_monster: 'Monstruo',
  munchkin_addMonster: '+ Agregar Monstruo',
  munchkin_equipment: 'Equipo',
  munchkin_modifiers: 'Modificadores',
  munchkin_helper: 'Ayudante',
  munchkin_helperEquipment: 'Equipo del Ayudante',
  munchkin_noHelper: 'Sin ayudante',
  munchkin_vs: 'vs',
  munchkin_youWin: '¡Ganas! 🎉',
  munchkin_monsterWins: 'El Monstruo GANA 💀',
  munchkin_levelsGained: 'Niveles ganados',
  munchkin_applyReward: 'Aplicar Recompensa',
  munchkin_cantWinBySelling: '¡No puedes llegar al Nivel 10 vendiendo o con cartas!',
  munchkin_wins: '¡{player} gana!',
  munchkin_adventurers: 'aventureros',
  munchkin_confirmWin: '¿Estás seguro de que {player} llegó al Nivel 10 y ganó la partida?',
  munchkin_lowestLevel: 'Más bajo',
  munchkin_levelsLost: 'Niveles perdidos (Bad Stuff)',
  munchkin_badStuffHint: '✓ = Escapaste · Elige niveles perdidos según el Bad Stuff del monstruo',
  munchkin_applyBadStuff: 'Aplicar Bad Stuff (−{levels})',
  munchkin_ranAway: '¡Escapaste! ✓',

  // Wrong Party
  wp_description: '¡Recluta invitados, combina el tema y organiza la fiesta más caótica para ganar!',
  wp_activeTheme: 'Tema Activo',
  wp_theme: 'Tema',
  wp_selectTheme: 'Seleccionar carta de tema…',
  wp_themeType: 'Tipo de tema',
  wp_themeAttributes: 'Atributos',
  wp_searchGuest: 'Buscar cartas de invitados…',
  wp_roundResult: 'Resultado de Ronda',
  wp_targetScore: 'Meta: 3 rondas',
  wp_hypnotistActive: 'Hipnotista Activo',
  wp_influencerAttr: 'Like del Influencer',
  wp_selectAttribute: 'Seleccionar atributo',
  wp_partyPlannerOverride: 'Organizador — Cambio de Tema',
  wp_declarePlannerType: 'Declarar tipo de tema…',
  wp_selectThemeFirst: 'Selecciona una carta de tema antes de terminar la ronda.',
  wp_showDetails: 'Detalles',
  wp_hideDetails: 'Ocultar',
  wp_bd_basePoints: 'Puntos base',
  wp_bd_themeBonus: 'Coincidencia de tema',
  wp_bd_likesPoints: 'Likes coincidentes',
  wp_bd_dislikesPoints: 'Dislikes coincidentes',
  wp_bd_boosterBonus: 'Cartas potenciadoras',
  wp_bd_synergyBonus: 'Bonificación de sinergia',
  wp_bd_rainbowBonus: 'Bonificación de arcoíris',

  // 7 Wonders Architects
  '7wa_description': '¡Construye tu maravilla y deja tu huella en la historia!',
  '7wa_chooseWonder': 'Elige tu maravilla',
  '7wa_startScoring': 'Iniciar Puntuación',
  '7wa_calculateScores': 'Calcular Puntuaciones',
  '7wa_wonderPoints': 'Puntos de Maravilla',
  '7wa_stagesConstructed': 'Fases Construidas',
  '7wa_bluePoints': 'Puntos Azules',
  '7wa_blueCatIcons': 'Iconos de Gato Azules',
  '7wa_militaryPoints': 'Puntos Militares',
  '7wa_militaryTokensCount': 'Fichas Militares',
  '7wa_progressPoints': 'Puntos de Progreso',
  '7wa_progressTokensCount': 'Fichas de Progreso',
  '7wa_catPawn': 'Peón de Gato',
  '7wa_stages': 'fases',
  '7wa_showResults': 'Mostrar Resultados',
  '7wa_tokenDecor': 'Arte',
  '7wa_tokenPolitics': 'Política',
  '7wa_tokenStrategy': 'Estrategia',
  '7wa_tokenEducation': 'Educación',
  '7wa_tokenCulture': 'Cultura',
  '7wa_next': 'Siguiente',
  '7wa_chooseTokens': 'Fichas de Progreso',
  '7wa_catPawnDesc': '+2 PV al final de la partida.',
  '7wa_artDesc': '+4 PV si la Maravilla está en construcción, +6 PV si está completada.',
  '7wa_politicsDesc': '+1 PV por icono de gato en tus cartas Azules.',
  '7wa_strategyDesc': '+1 PV por marcador de victoria militar.',
  '7wa_educationDesc': '+2 PV por cada ficha de Progreso que tengas.',
  '7wa_cultureDesc': '+4 PV por 1 ficha, +12 PV por las dos.',

  // Everdell
  everdell_description: '¡Construye una ciudad de criaturas y construcciones en el valle de Everdell!',
  everdell_calculateScores: 'Calcular Puntuaciones',
  everdell_baseCardPoints: 'Puntos de Cartas',
  everdell_pointTokens: 'Fichas de Puntos',
  everdell_prosperityBonus: 'Bonus de Prosperidad',
  everdell_journeyPoints: 'Puntos de Viaje',
  everdell_eventPoints: 'Puntos de Eventos',
  everdell_eventsAchieved: 'Eventos Logrados',
  everdell_leftoverResources: 'Recursos Sobrantes',
  everdell_tiebreakHint: 'Desempate: más eventos → más recursos',
  everdell_showResults: 'Mostrar Resultados',
  everdell_events: 'eventos',

  // Harmonies
  harmonies_description: '¡Coloca fichas de terreno y completa patrones de animales en un hermoso puzzle de paisajes!',
  harmonies_calculateScores: 'Calcular Puntuaciones',
  harmonies_treePoints: 'Árboles 🌲',
  harmonies_mountainPoints: 'Montañas ⛰️',
  harmonies_fieldPoints: 'Campos 🌾',
  harmonies_buildingPoints: 'Edificios 🏠',
  harmonies_riverPoints: 'Ríos 💧',
  harmonies_animalPoints: 'Cartas de Animales 🐾',
  harmonies_tiebreakTitle: 'Empate — Desempate',
  harmonies_animalCubesPlaced: 'Cubos colocados',
  harmonies_completedCards: 'Cartas completadas',
  harmonies_unplacedCubes: 'Cubos restantes',
  harmonies_tiebreakHint: 'Más cubos colocados → más cartas completadas → menos cubos sin colocar',
  harmonies_showResults: 'Mostrar Resultados',
  harmonies_cubesPlaced: 'cubos colocados',
  harmonies_addCard: 'Agregar carta',
  harmonies_noCards: 'Toca + Agregar carta para ingresar cada carta de animal.',
  harmonies_cardLabel: 'Carta',
  harmonies_scoringRef: 'Referencia de Puntuación',
  harmonies_ref_trees: 'Altura 1 = 3 pts · Altura 2 = 7 pts · Altura 3 = 12 pts. Suma todos los árboles.',
  harmonies_ref_mountains: 'Altura 1 = 1 pt · Altura 2 = 3 pts · Altura 3 = 7 pts. Debe estar adyacente a otra montaña o vale 0 pts.',
  harmonies_ref_fields: 'Cada grupo distinto de 2+ fichas amarillas contiguas = 5 pts.',
  harmonies_ref_buildings: 'Cada edificio rodeado por 3+ fichas de colores diferentes = 5 pts.',
  harmonies_ref_rivers: 'Río más largo: 1→1, 2→2, 3→4, 4→7, 5→11, 6→15, luego +4 por ficha más allá de 6.',
  harmonies_ref_animals: 'Suma los PV indicados en el espacio superior sin cubo de animal. Una carta con todos sus cubos vale 0 puntos.',

  // BGG Integration
  logToBgg: 'Registrar en BGG',
  logPlay: 'Registrar Partida',
  logPlayTitle: 'Registrar una Partida',
  logPlayDescription: 'Guarda una sesión de juego en BoardGameGeek',
  bggSettings: 'Cuenta BGG',
  bggUsername: 'Usuario BGG',
  bggPassword: 'Contraseña BGG',
  bggPin: 'PIN de desbloqueo',
  bggPinPlaceholder: 'Ingresa tu PIN',
  bggPinPromptDesc: 'Ingresa tu PIN para descifrar tus credenciales de BGG.',
  bggPinError: 'PIN incorrecto o no se encontraron credenciales.',
  bggRememberMe: 'Recordar credenciales (encriptadas)',
  bggLogin: 'Conectar a BGG',
  bggFillAllFields: 'Por favor completa usuario, contraseña y PIN.',
  bggLogout: 'Desconectar',
  bggDisclaimer: 'BGG no ofrece una API oficial para registrar partidas. Esta función usa un endpoint interno que puede cambiar sin previo aviso.',
  bggAttribution: 'Datos de juegos provistos por la API XML de BoardGameGeek.',
  bggLoginSuccess: '¡Conectado a BGG exitosamente!',
  bggLoginError: 'Error al conectar a BGG',
  bggPlayLogged: '¡Partida registrada en BGG! 🎲',
  bggPlayError: 'No se pudo registrar la partida en BGG',
  bggRetry: 'Reintentar',
  bggNotConfigured: 'Configura tu cuenta BGG en Ajustes primero.',
  bggGoToSettings: 'Ir a Ajustes',
  bggNoGameId: 'Este juego no tiene ID de BGG configurado.',
  bggConnected: 'Conectado',
  bggNotConnected: 'No conectado',
  bggTestConnection: 'Probar Conexión',
  bggClearCredentials: 'Borrar Credenciales Guardadas',
  bggCredentialsSaved: 'Credenciales guardadas y encriptadas.',
  bggCredentialsCleared: 'Credenciales borradas.',
  bggSessionActive: 'Sesión activa en esta pestaña.',
  bggLinkPlayers: 'Vincular Jugadores a BGG',
  bggResolved: 'Vinculado',
  bggUnresolved: 'Ingresa usuario de BGG',
  bggSelectProfile: 'Selecciona o escribe usuario BGG',
  // Log Play page
  searchGames: 'Buscar juegos…',
  selectGame: 'Seleccionar un juego',
  enterScores: 'Ingresar puntuaciones',
  reviewAndSubmit: 'Revisar y Enviar',
  logPlayStep: 'Paso {step} de {total}',
  logPlayNoResults: 'No se encontraron juegos. Intenta otra búsqueda.',
  logPlaySelectGame: 'Seleccionar Juego',
  logPlayAddPlayers: 'Agregar Jugadores',
  logPlayEnterScores: 'Ingresar Puntuaciones',
  logPlayReview: 'Revisar',
  logPlaySubmit: 'Registrar Partida en BGG',
  logPlaySuccess: '¡Partida guardada en BGG!',
  logPlayBack: 'Volver',
  logPlayNext: 'Siguiente',
  winnerLabel: 'Ganador',
  autoWinner: 'El puntaje más alto gana automáticamente',
  // LogPlayPage sections & fields
  logPlayGameSection: '🎲 Juego',
  logPlayDetailsSection: '📅 Detalles',
  logPlayPlayersSection: '👥 Jugadores',
  logPlayDate: 'Fecha',
  logPlayLocation: 'Lugar (opcional)',
  logPlayLength: 'Duración (minutos, opcional)',
  logPlayTeamColor: 'Equipo / Color',
  logPlayBggUsername: 'Usuario BGG (opcional)',
  logPlayFirstTime: 'Primera vez jugando',
  logPlaySearchPlaceholder: 'Buscar un juego en BGG…',
  logPlaySelectPrompt: 'Selecciona un juego para registrar',
  logPlayLocationHint: 'Empieza a escribir para ver lugares recientes',
  logPlayValidationError: 'Por favor selecciona un juego y agrega al menos un jugador.',
  // Colores de equipo
  colorPurple: 'Morado',
  colorTeal: 'Verde azulado',
  colorPink: 'Rosa',
  colorAmber: 'Ámbar',
  colorBlue: 'Azul',
  colorGreen: 'Verde',
  colorRed: 'Rojo',
  colorOrange: 'Naranja',
};


export default es;
